...
import PiCameraFilter from './components/PiCameraFilter';
import { 
  ChartBarIcon, 
  MusicalNoteIcon, 
  SunIcon, 
...
function App() {
  ...
  const featureDetails = {
    analysis: {
      title: "Discover Hidden Patterns in π",
      description: "Leverage advanced AI algorithms to analyze and visualize patterns, frequencies, and anomalies in the digits of π.",
      gradient: "from-purple-600 to-pink-600"
    },
    fractal: {
      title: "Create Mathematical Art with π",
      description: "Transform π's infinite digits into stunning fractal visualizations using advanced mathematical algorithms.",
      gradient: "from-blue-600 to-purple-600"
    },
    music: {
      title: "Listen to the Music of π",
      description: "Convert π's digits into beautiful musical compositions across different styles and instruments.",
      gradient: "from-green-600 to-blue-600"
    },
    camera: {
      title: "Capture Moments with π",
      description: "Add interactive π-themed filters and effects to your photos in real-time.",
            ...
            <div className="flex items-center justify-between h-16">
              ...
              <div className="flex items-center space-x-2">
                ...
                <motion.button
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.95 }}
                  onClick={() => setCurrentFeature('camera')}
                  className={`px-4 py-2 rounded-lg transition-colors flex items-center space-x-2 ${
        ...
        <main className="container mx-auto px-4 sm:px-6 lg:px-8 py-12">
          ...
          >
            <h2 className="text-4xl font-extrabold text-gray-900 dark:text-white sm:text-5xl">
              {featureDetails[currentFeature].title.split('π').map((part, i, arr) => (
                <span key={i}>
                  {part}
                  {i < arr.length - 1 && (
                    <span className={`text-transparent bg-clip-text bg-gradient-to-r ${featureDetails[currentFeature].gradient}`}>
                      π
                    </span>
                  )}
                </span>
            ...
            <p className="mt-4 text-xl text-gray-600 dark:text-gray-300">
              {featureDetails[currentFeature].description}
            </p>
          </motion.div>
          
          {currentFeature === 'analysis' ? <PiAnalysis /> : 
           currentFeature === 'fractal' ? <PiFractal /> : 
           currentFeature === 'music' ? <PiMusic /> : 
           currentFeature === 'camera' ? <PiCameraFilter /> : null}
        </main>
          ...
          <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <div className="text-center">
              <p className="text-sm text-gray-600 dark:text-gray-400">
                © {new Date().getFullYear()} Pi-Matrix AI. Made with ❤️ by Nithin. All rights reserved.
              </p>
              <div className="mt-2 flex justify-center space-x-6">
                <a href="https://github.com/nithin-developer" className="text-gray-500 hover:text-purple-600 dark:hover:text-purple-400 transition-colors duration-300">
